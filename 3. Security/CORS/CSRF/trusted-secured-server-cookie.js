...
function validateCsrfTokenFromCookie(req, res, next) {
  const allCookies = req.cookies; // Unsigned cookies
...
app.post('/transfer', validateCsrfTokenFromCookie, (req, res) => {
  console.log('Payment done');
  const { to, amount } = req.body;
  balance -= parseInt(amount, 10);
  res.send(`<h1>Transferred $${amount} to ${to}. Your new balance is $${balance}.</h1>`);
...
app.listen(5004, () => console.log('Bank Server started at 5004'));
